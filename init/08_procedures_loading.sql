...
CREATE OR REPLACE PACKAGE pkg_carga_datos AS
    
    PROCEDURE sp_carga_general(p_fecha_carga DATE);
    
    PROCEDURE sp_cargar_agentes(p_fecha_carga DATE);
    PROCEDURE sp_cargar_clientes(p_fecha DATE);
    PROCEDURE sp_cargar_productos(p_fecha DATE);
    PROCEDURE sp_cargar_categorias(p_fecha DATE);
    PROCEDURE sp_cargar_canales(p_fecha DATE);
    
    PROCEDURE sp_cargar_fact_support(p_fecha DATE);
    
    PROCEDURE sp_limpiar_modelo;
    PROCEDURE sp_pipeline_carga_soporte;
    
END pkg_carga_datos;
/

CREATE OR REPLACE PACKAGE BODY pkg_carga_datos AS

    PROCEDURE sp_carga_general(p_fecha_carga DATE) AS
    BEGIN
        INSERT INTO control_procesos(nombre_tabla, operacion, fecha_carga_parametro)
        VALUES ('CONTROL_PROCESOS', 'INSERT', p_fecha_carga);
    END sp_carga_general;

    PROCEDURE sp_cargar_agentes(p_fecha_carga DATE) IS
        v_filas NUMBER;
    BEGIN
    ...
    END sp_cargar_agentes;

    PROCEDURE sp_cargar_clientes(p_fecha DATE) IS
        v_filas NUMBER;
    BEGIN
    ...
    END sp_cargar_clientes;

    PROCEDURE sp_cargar_productos(p_fecha DATE) IS
        v_filas NUMBER;
    BEGIN
    ...
    END sp_cargar_productos;

    PROCEDURE sp_cargar_categorias(p_fecha DATE) IS
        v_filas NUMBER;
    BEGIN
    ...
    END sp_cargar_categorias;

    PROCEDURE sp_cargar_canales(p_fecha DATE) IS
        v_filas NUMBER;
    BEGIN
    ...
    END sp_cargar_canales;

    PROCEDURE sp_cargar_fact_support(p_fecha DATE) IS
        v_filas NUMBER;
    BEGIN
        ...
        USING (
            SELECT
                ...
                TO_TIMESTAMP(t.issue_reported_at, 'DD-MM-YYYY HH24:MI') AS issue_reported_at,
                TO_TIMESTAMP(t.issue_responded, 'DD-MM-YYYY HH24:MI') AS issue_responded,
    ...
    END sp_cargar_fact_support;

    PROCEDURE sp_limpiar_modelo IS
    BEGIN
    ...
    END sp_limpiar_modelo;

    PROCEDURE sp_pipeline_carga_soporte IS
